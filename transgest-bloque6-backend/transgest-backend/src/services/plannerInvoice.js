...
async function saleLines(tx,company,prepId,customer){
 ...
 const lines=(await tx.query(`SELECT l.referencia || ' · ' || l.descripcion || ' · lote ' || COALESCE(NULLIF(e.lote,''),'—') || CASE WHEN l.descuento_pct>0 THEN ' · descuento ' || l.descuento_pct || '%' ELSE '' END AS concepto,
  l.cantidad,ROUND(l.precio_venta*(1-l.descuento_pct/100),4) AS precio_unit FROM planner_preparacion_lineas l JOIN planner_existencias e ON e.id=l.existencia_id AND e.empresa_id=l.empresa_id
