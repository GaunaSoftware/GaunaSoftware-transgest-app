...
Fecha de revisión: 23/09/2026. Rama aislada `codex/bi-phase1-reliability`. **El push de la rama no es un despliegue ni una aprobación de producción.** Las pruebas de cálculo y envío usan datos sintéticos. Después se comprobó, con autorización expresa y solo en lectura, el acceso y los posibles destinatarios de Asensi; no se ejecutó allí el código nuevo.
...
| Regresiones operativas | `npm run check` backend y suite frontend cubren pedidos, tráfico, facturación, chofer, hojas de ruta y Planner | Suite frontend: 28/28 y 79/79; `npm run check` backend terminó con código 0 |
...
3. El informe semanal solicitado conserva el acceso a demanda para gerencia. La ampliación posterior añade programación opt-in cada lunes a las 09:00 de Madrid con PDF y trazas por destinatario. Calcula la semana completa anterior, con costes directos, margen y km por camión. **No se presenta como beneficio neto**. El correo automático no se ha activado en producción.
...
- El envío semanal añadido después de la auditoría requiere migración, selección expresa de destinatarios, SMTP operativo y validación integrada en un entorno de ensayo. El estado `por_verificar` evita repetir automáticamente un envío cuyo resultado SMTP es ambiguo. Si el proceso no está activo ningún lunes, no existe recuperación diferida automática; esa semana debe generarse a demanda.

## Migración y ensayo previo al despliegue

Hay dos migraciones BI aditivas: `transgest-backend/scripts/migrations/20260923_bi_report_center.sql` crea vistas, ejecuciones y exportaciones privadas; `20260923_bi_weekly_delivery.sql` crea suscripciones y trazas del envío semanal. No alteran facturas ni pedidos. Se aplicaron solo a PGlite durante las pruebas.
...
| Frontend: `CI=true; npm test -- --watch=false --runInBand` | 28 suites y 79 pruebas, todas pasaron tras añadir dos casos de programación semanal. |
...
## Comprobación posterior del envío semanal

El usuario solicitó expresamente probar con la cuenta de Asensi. El acceso de solo lectura confirmó que `trafico@transportesasensi.com` corresponde a MANUEL BERRUECO, usuario activo con rol Gerente, en TRANSPORTES ASENSI, S.L.U.; el plan mostrado es Pro Intelligence y hay otro Gerente activo. El correo general de empresa está vacío. El destinatario del PDF debe seleccionarse expresamente en la nueva configuración; no se deduce de ese campo ni de la lista de albaranes. La versión productiva visitada todavía no ofrece el Centro de informes de esta rama. No se aplicó la migración, no se activó la suscripción ni se envió correo real.

Antes de habilitarlo en una instalación: aplicar ambas migraciones BI y comprobar su registro; entrar con gerencia; abrir Informes → Centro de informes; elegir los Gerentes destinatarios y guardar; generar el informe a demanda y reconciliar PDF contra pedidos y costes conocidos; probar envío con una cuenta de ensayo y SMTP autorizado; revisar el estado del primer lunes (`enviado`, `sin_smtp`, `fallido` o `por_verificar`) y el registro `email_log`. Una entrega fallida o ambigua requiere revisión humana y puede generarse a demanda. No marcar como listo para producción mientras sigan los límites financieros y de integración indicados arriba.

## Preparación posterior para despliegue autorizado (23/09/2026)

El usuario autorizó iniciar un despliegue, pero la verificación de respaldo y de la base productiva sigue siendo un punto de control. El comando `npm start` del backend no ejecuta el migrador versionado; se incorporó la aplicación idempotente de las dos migraciones BI aditivas al arranque, con checksum. Si fallan, `/health` devuelve 503 degradado. La batería `npm run check` y la prueba específica de migración doble sobre PGlite terminaron con código 0. Antes de dar por completado el despliegue se debe confirmar un respaldo restaurable, CI del commit final, release del backend, estado saludable de `/health`, tablas BI registradas y acceso funcional al Centro de informes. El correo semanal permanece desactivado hasta que un Gerente elija expresamente destinatarios; no se ha enviado ningún PDF real.
