  ...
  return (
    <div className="tg-chofer-section-shell" style={{padding:"12px 16px"}}>
      <div className="tg-chofer-card" style={{background:"var(--bg2)",border:"1px solid var(--border)",borderRadius:10,padding:14}}>
        ...
        {step===2 && <section className="driver-review"><h3>Revisa el viaje</h3><dl>{[["Cliente",form.cliente_nombre],["Origen",form.origen],["Destino",form.destino],["Carga",[form.fecha_carga,form.hora_carga].filter(Boolean).join(" · ")],["Descarga",[form.fecha_descarga,form.hora_descarga].filter(Boolean).join(" · ")],["Mercancía",form.mercancia],["Peso (kg)",form.peso_kg],["Bultos",form.bultos],["Notas",form.notas]].map(([label,value])=><div key={label}><dt>{label}</dt><dd>{value||"—"}</dd></div>)}</dl></section>}
