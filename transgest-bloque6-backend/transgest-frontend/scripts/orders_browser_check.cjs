...
async function main(){fs.mkdirSync(out,{recursive:true});
 ...
 try {
  ...
  assert.equal(await page.locator('.orders-list-card tbody tr[id]').count(),10);
  await page.getByLabel('Página siguiente de pedidos').click();assert.equal(await page.locator('.orders-list-card tbody tr[id]').count(),4);
  ...
  await page.getByLabel('Estado del pedido').first().selectOption('incidencia');await page.waitForFunction(()=>document.querySelectorAll('.orders-list-card tbody tr[id]').length===2);
  await page.getByRole('button',{name:'Restablecer filtros'}).click();await page.waitForFunction(()=>document.querySelectorAll('.orders-list-card tbody tr[id]').length===10);
