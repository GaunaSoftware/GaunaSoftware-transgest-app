...
async function prepare(db,company,user,input){
 ...
 return db.transaction(async tx=>{
  ...
  if(order.origen_producto!=='planner')throw fail('Selecciona una carga creada en Planner. Los viajes de TransGest se gestionan en su espacio.',409);
  ...
  const lines=input.lineas.map(line=>{
   ...
   const discount=decimal(line.descuento_pct??0);if(discount>100)throw fail('El descuento debe estar entre 0 y 100 %.');
  ...
  for(const line of lines){
   ...
   totalWeight+=Number(row.peso_kg)*line.cantidad;if(row.unidad==='palet')supportPallets+=line.cantidad;else totalPallets+=Math.ceil(line.cantidad/Number(row.unidades_palet));descriptions.push(`${row.referencia} · ${row.descripcion}`);
