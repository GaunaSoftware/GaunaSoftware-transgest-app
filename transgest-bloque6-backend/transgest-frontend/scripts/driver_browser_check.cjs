...
const driver={id:'driver-1',nombre:'Conductor',apellidos:'de prueba',firma_base:image,vehiculo_id:'truck-1',vehiculo_matricula:'1234-BCD',vehiculo_remolque_id:'trailer-1',remolque_matricula:'R-1234',gps_provider:'external',gps_external_id:'qa',km_actuales:123000};
let jornada={km_inicio:123000,vehiculo_id:'truck-1',id:'day-1',estado:'abierta',inicio_at:'2026-09-12T05:00:00Z',actividad_actual:'otros_trabajos',resumen:{calculado_at:'2026-09-12T08:00:00Z',conduccion_min:90,conduccion_desde_pausa_min:90,avisos:[]},eventos:[{tipo:'otros_trabajos',at:'2026-09-12T05:00:00Z'}]};
...
async function main(){
 ...
 try{
  ...
  await page.route('**/api/v1/**',async route=>{
   ...
   else if(p==='/pedidos/chofer/clientes')data=[{id:'client-1',nombre:'Cliente de prueba'}];
   else if(p.includes('/puntos-carga'))data=[{id:'point-1',nombre:'Valencia',direccion:'Calle Uno',ciudad:'Valencia',tipo:'carga'}];
   else if(p==='/pedidos/chofer')data={id:'created',documento_control:{qr:{data_url:image}}};
   else if(p.includes('chofer-pasos'))data={data:{}};
   else if(p.includes('documento-control'))data={status:{ready:false,faltantes:['Destino']},documento:{soporte_url:'https://example.com/qa-dcd'},qr:{data_url:image}};
   else if(p==='/choferes/app/jornada')data={chofer:driver,jornada};
   else if(p==='/choferes/app/jornada/cerrar'){jornada=null;data={ok:true};}
   else if(p==='/choferes/app/jornada/iniciar'){jornada={estado:'abierta',km_inicio:body.km_inicio,vehiculo_id:driver.vehiculo_id,id:'day-2',inicio_at:'2026-09-12T08:00:00Z',actividad_actual:'otros_trabajos',resumen:{},eventos:[]};data={ok:true};}
   else if(p==='/choferes/app/jornada/actividad'){jornada.actividad_actual=body.actividad;data={ok:true};}
   else if(p==='/choferes/app/conjunto'&&req.method()!=='GET'&&jornada&&body.vehiculo_id!==driver.vehiculo_id)return route.fulfill({status:409,json:{error:'Cierra la jornada de la tractora actual antes de cambiar de vehículo y abre otra con su cuentakilómetros.'}});
  ...
  await page.getByRole('heading',{name:'Iniciar sesión'}).waitFor();assert.equal(await page.getByText(/TransGest Pro Intelligence/i).count(),0);
  for(const width of [375,390,393,430,768]){await page.setViewportSize({width,height:900});assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth+1),false);await page.screenshot({path:path.join(out,`login-${width}.png`),fullPage:true});}
  ...
  async function navigate(tab){
   if(tab==='Activos'){await bottom.getByRole('button',{name:'Mis viajes',exact:true}).click();return;}
   if(tab==='Jornada'){await bottom.getByRole('button',{name:'Jornada',exact:true}).click();return;}
   await bottom.getByRole('button',{name:'Más',exact:true}).click();await page.locator('.driver-menu').getByRole('button',{name:tab,exact:true}).click();
  }
  for(const theme of ['light','dark']){
   ...
   for(const tab of ['Activos','Nuevo','Jornada','Datos','Vacaciones','Historial','Taller']){
    await navigate(tab);
    for(const width of [375,390,393,430,768]){await page.setViewportSize({width,height:900});await page.waitForTimeout(100);assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth+1),false,`${tab} overflow ${width}`);await page.screenshot({path:path.join(out,`${tab}-${theme}-${width}.png`),fullPage:true});}
   }
  }
  checks.push('All seven sections, light/dark themes and mobile/tablet layouts');
  await navigate('Nuevo');await page.getByRole('button',{name:'Siguiente: Carga y horarios',exact:true}).scrollIntoViewIfNeeded();await bottom.getByRole('button',{name:'Jornada',exact:true}).click();await page.waitForTimeout(100);const headingBox=await page.locator('.driver-page-heading').boundingBox();assert(headingBox.y>=0&&headingBox.y<300,'bottom navigation must return to the section header');
  ...
  await navigate('Jornada');await page.locator('.driver-set-editor summary').click();await page.getByLabel('Tractora',{exact:true}).selectOption('truck-2');await page.getByRole('button',{name:'Actualizar conjunto',exact:true}).click();await page.getByText('Cierra la jornada de la tractora actual antes de cambiar de vehículo y abre otra con su cuentakilómetros.',{exact:true}).waitFor();await page.getByLabel('Tractora',{exact:true}).selectOption('truck-1');
  ...
  await page.locator('.driver-set-confirm input').check();await page.getByLabel('Kilómetros al terminar').fill('123000');await page.getByRole('button',{name:'Cerrar jornada',exact:true}).click();assert(!writes.some(w=>w.p.endsWith('/jornada/cerrar')),'Equal odometer must not submit');await page.getByLabel('Kilómetros al terminar').fill('123050');await page.getByRole('button',{name:'Cerrar jornada',exact:true}).click();await page.getByRole('button',{name:'Iniciar jornada',exact:true}).waitFor();await page.locator('.driver-set-confirm input').check();await page.getByLabel('Kilómetros al iniciar').fill('123050');await page.getByRole('button',{name:'Iniciar jornada',exact:true}).click();await page.getByText('Jornada en curso',{exact:true}).waitFor();checks.push('Live day summary, vehicle assignment and day activity/start/end API calls');
  ...
  await navigate('Datos');await page.getByRole('button',{name:'Cambiar firma',exact:true}).click();await page.locator('canvas').waitFor();await page.screenshot({path:path.join(out,'signature.png')});await page.getByRole('button',{name:'Firmar',exact:true}).click();assert.equal(writes.filter(w=>w.p==='/choferes/app/firma-base').length,0,'empty signature must not save');await drawSignature();await page.getByRole('button',{name:'Firmar',exact:true}).click();await page.locator('canvas').waitFor({state:'hidden'});assert(writes.some(w=>w.p==='/choferes/app/firma-base'));
  await navigate('Vacaciones');await page.getByLabel('Inicio de vacaciones').fill('2026-10-01');await page.getByLabel('Fin de vacaciones').fill('2026-10-05');await page.getByRole('button',{name:'Solicitar y firmar',exact:true}).click();await page.locator('canvas').waitFor();await drawSignature();await page.getByRole('button',{name:'Firmar',exact:true}).click();await page.locator('canvas').waitFor({state:'hidden'});assert(writes.some(w=>w.p==='/choferes/app/vacaciones'&&w.body.fecha_fin==='2026-10-05'));checks.push('Personal/holiday signatures, rejecting empty signatures and saving signed data');
  await navigate('Nuevo');await page.getByLabel('Cliente / destinatario').fill('Cliente de prueba');await page.getByRole('button',{name:'Cliente de prueba',exact:true}).click();await page.getByText('Puntos de carga del cliente',{exact:true}).waitFor();assert.equal(await page.getByLabel('Origen / punto de carga').inputValue(),'','Single point must require explicit selection');await page.getByLabel('Origen / punto de carga').fill('Valencia');await page.getByLabel('Destino / punto de descarga').fill('Madrid');await page.getByRole('button',{name:'Siguiente: Carga y horarios',exact:true}).click();await page.getByLabel('Mercancía',{exact:true}).fill('Palets');await page.getByRole('button',{name:'Siguiente: Revisión',exact:true}).click();await page.getByRole('button',{name:'Crear viaje y DCD',exact:true}).click();await page.getByText('QR generado',{exact:true}).waitFor();assert(writes.some(w=>w.p==='/pedidos/chofer'&&w.body.mercancia==='Palets'));
  await navigate('Taller');await page.getByRole('button',{name:'Neumático pinchado',exact:true}).click();await page.getByRole('button',{name:'Enviar solicitud de taller',exact:true}).click();await page.getByText('Solicitud enviada',{exact:true}).waitFor();assert(writes.some(w=>w.p==='/taller/solicitudes'&&w.body.motivo==='neumatico_pinchado'));checks.push('Create trip/DCD and workshop request');
  await navigate('Activos');await page.getByRole('button',{name:'Ver detalles del viaje P-QA-001',exact:true}).press('Enter');await page.waitForTimeout(200);await page.getByRole('button',{name:'Ver DCD',exact:true}).waitFor();assert.equal(await page.getByLabel('Ej: 125000').count(),0);await page.screenshot({path:path.join(out,'trip-expanded.png'),fullPage:true});
  failList=true;await bottom.getByRole('button',{name:'Más',exact:true}).click();await page.getByRole('button',{name:'Actualizar información'}).click();await page.locator('.driver-load-error').waitFor();failList=false;await page.locator('.driver-load-error').getByRole('button',{name:'Reintentar'}).click();await page.locator('.driver-load-error').waitFor({state:'hidden'});
