...
- Implementada exportacion de inmovilizado dentro del paquete ZIP para asesoria, con permiso `fixed_assets.read` y CSV fisico `exports/inmovilizado.csv`.
- Implementada exportacion de estado de periodos dentro del paquete ZIP para asesoria, con permiso `periods.read` y CSV fisico `exports/periodos.csv`.
...
- Implementado guard compartido de Diario para rechazar creacion, edicion, reverso y contabilizacion cuando el ejercicio o periodo destino no estan abiertos.
- Implementado bloqueo equivalente en staging externo: los lotes de cuentas y Diario aprobados no se aplican si el ejercicio destino no esta abierto, y los asientos importados exigen periodo abierto.
- Implementada previsualizacion preventiva de staging externo: cuentas y Diario muestran errores por ejercicio cerrado, periodo inexistente o periodo no abierto antes de aplicar el lote.
...
- Implementado bloqueo de altas, cambios de estado e importaciones de plantillas de cuentas cuando el ejercicio destino no esta abierto.
...
- Implementado CSV de inmovilizado en el paquete de asesoria, junto al manifiesto y al indice del ZIP.
...
- Implementado bloqueo de cambios ordinarios del plan contable en ejercicios cerrados.
