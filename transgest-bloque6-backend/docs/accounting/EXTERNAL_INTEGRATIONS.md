...
- La previsualizacion y aplicacion controlada de cuentas contables reutiliza el mismo endpoint con `import_type=accounts` y `fiscal_year_id` explicito. Crea cuentas solo en el ejercicio seleccionado, bloquea codigos duplicados y avisa/rechaza si el ejercicio destino no esta abierto.
...
- La previsualizacion y aplicacion controlada de diario reutiliza el mismo endpoint con `import_type=journal_entries` y `fiscal_year_id` explicito. Agrupa lineas por referencia externa, exige partida doble cuadrada, detecta periodo destino, avisa/rechaza ejercicios o periodos no abiertos y crea solo borradores de diario trazables. No contabiliza asientos automaticamente.
...
- No crea cuentas bancarias, facturas ni asientos; terceros, cuentas, vencimientos y movimientos bancarios solo se crean desde endpoints controlados de aplicacion de lotes aprobados.
