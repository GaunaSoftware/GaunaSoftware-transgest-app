...
function QuickInfo({ label, text }) { return <details className="orders-quick-info" onClick={e=>e.stopPropagation()}><summary title={text}>{label}</summary><div style={{whiteSpace:"pre-line"}}>{text}</div></details>; }
...
export default function OrdersWorkspace({ vehicles = [], items, allItems, loading, error, reload, clients, drivers, labels, filters, actions, permissions, selectedIds, toggleSelected, tools, bulkTools, serverPage, serverPages, totalCount, setServerPage, describe }) {
  ...
  const columns=[{key:"selection",label:<span className="orders-sr">Selección</span>,render:({pedido:p})=>check(p)},
    ...
    {key:"origin",label:"Origen",render:({pedido:p})=>{const r=describe(p);return <QuickInfo label={<>{r.origin}{r.loads>1&&<small>+{r.loads-1} cargas</small>}</>} text={r.loadDetails || r.origin}/>;}},
    {key:"destination",label:"Destino",render:({pedido:p})=>{const r=describe(p);return <QuickInfo label={<>{r.destination}{r.unloads>1&&<small>+{r.unloads-1} descargas</small>}</>} text={r.unloadDetails || r.destination}/>;}},
    ...
    {key:"vehicle",label:"Matrícula",render:({pedido:p})=><>{orderRig(p,vehicles)}{p.colaborador_id&&<small>{p.colaborador_nombre || "Colaborador"}</small>}</>},
    ...
    {key:"incident",label:"Incidencia",className:"orders-incident-cell",render:({pedido:p,priorityMeta:m})=>incidentLabel(p)?<QuickInfo label={<span className="orders-incident"><Icon name="alert" size={14}/>{incidentLabel(p)}</span>} text={incidentDescription(p)}/>:m.validationIssues.length?<span title={m.validationIssues.join(" · ")} className="orders-warning">Datos pendientes ({m.validationIssues.length})</span>:"—"},
