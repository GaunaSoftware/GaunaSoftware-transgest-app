...
if(p==='/auth/me')data=user;else if(p.includes('notificaciones'))data={data:[],no_leidas:0};else if(p==='/pedidos'||p==='/pedidos/resumen-lista')data=[{id:'qa-info',numero:'QA-INFO',chofer_id:'driver-a',vehiculo_id:'truck-a',vehiculo_matricula:'1234 ABC',remolque_matricula:'R-1234',estado:'incidencia',fecha_carga:today,origen:'Madrid',destino:'Valencia',puntos_carga:[{direccion:'Madrid',hora:'08:30',ventana:'08:00–10:00'},{direccion:'Carga adicional Toledo'}],puntos_descarga:[{direccion:'Valencia'},{direccion:'Descarga adicional Alicante',fecha:'2026-09-16',hora:'14:00',ventana:'14:00–16:00'}],incidencia_descripcion:'Retraso por avería en carretera'}];else if(p==='/choferes')data=[driver];else if(p==='/portal-cliente/puntos'){if(body){const point={...body,id:'point-'+points.length};points.push(point);data=point;}else data=points;}else if(p==='/portal-cliente/calcular-ruta')data={km:350,warning:'Estimación QA'};else if(p==='/portal-cliente/solicitudes'&&body)data={id:'request-qa'};else if(p.includes('resumen'))data={};else if(p.includes('empresa'))data={plan:'enterprise'};
...
await load.hover();await page.getByRole('tooltip').filter({hasText:'08:30'}).waitFor();await page.getByRole('tooltip').filter({hasText:'Carga adicional Toledo'}).waitFor();await page.keyboard.press('Escape');
const unload=row.getByRole('button').filter({hasText:'+1 descarga'});
await unload.hover();await page.getByRole('tooltip').filter({hasText:'Ventana: 14:00–16:00'}).waitFor();await page.getByRole('tooltip').filter({hasText:'Descarga adicional Alicante'}).waitFor();await page.keyboard.press('Escape');
const incident=row.getByRole('button',{name:'Incidencia operativa',exact:true});
await incident.hover();await page.getByRole('tooltip').filter({hasText:'Retraso por avería'}).waitFor();await page.keyboard.press('Escape');
await incident.click();
await page.getByRole('tooltip').filter({hasText:'Retraso por avería en carretera'}).waitFor();
await load.click();await page.getByRole('tooltip').filter({hasText:'2. Carga adicional Toledo'}).waitFor();
