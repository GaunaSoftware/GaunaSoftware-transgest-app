...
export default function IntelligenceAnswer({content}) {
  ...
  for (let i=0; i<lines.length;) {
    ...
    if (i+1<lines.length && line.includes('|') && divider(lines[i+1])) {
      ...
      blocks.push(<div className="intelligence-table-scroll" key={"table-"+i} tabIndex={0} role="region" aria-label="Resultados de la consulta"><table><thead><tr>{header.map((v,j)=><th key={j} scope="col">{inline(v)}</th>)}</tr></thead><tbody>{rows.map((row,r)=><tr key={r}>{header.map((_,c)=><td key={c}>{inline(row[c] || '—')}</td>)}</tr>)}</tbody></table></div>);
      continue;
    }
    const heading = line.match(/^#{1,4}\s+(.+)$/);
    if (heading) { blocks.push(<h3 key={"heading-"+i}>{inline(heading[1])}</h3>); i++; continue; }
    ...
    if (list) {
      ...
      blocks.push(<Tag key={"list-"+i}>{items.map((v,j)=><li key={j}>{inline(v)}</li>)}</Tag>); continue;
    }
    blocks.push(<p key={"paragraph-"+i}>{inline(line)}</p>); i++;
  }
