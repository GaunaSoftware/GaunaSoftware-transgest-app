...
export default function OrdersWorkspace({ vehicles = [], items, allItems, loading, error, reload, clients, drivers, labels, filters, actions, permissions, selectedIds, toggleSelected, tools, bulkTools, serverPage, serverPages, totalCount, setServerPage, describe }) {
  ...
  const columns=[{key:"selection",label:<span className="orders-sr">Selección</span>,render:({pedido:p})=>check(p)},
    ...
    {key:"state",label:"Estado",render:({pedido:p})=><><span title={incidentDescription(p)}><Badge tone={stateTone(p.estado)}>{labels[p.estado] || p.estado || "Sin estado"}</Badge></span>{isLocked(p)&&<small>Facturado</small>}{p.estado==="cancelado"&&p.motivo_cancelacion&&<small>{p.motivo_cancelacion}</small>}</>},
    {key:"incident",label:"Incidencia",render:({pedido:p,priorityMeta:m})=>incidentLabel(p)?<QuickInfo label={<span className="orders-incident"><Icon name="alert" size={14}/>{incidentLabel(p)}</span>} text={incidentDescription(p)}/>:m.validationIssues.length?<span title={m.validationIssues.join(" · ")} className="orders-warning">Datos pendientes ({m.validationIssues.length})</span>:"—"},
