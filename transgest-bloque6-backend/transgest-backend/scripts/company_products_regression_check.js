...
async function main(){
  const companies=new Set(['a','b']), modes=new Map(),plans=new Map();let schemaCalls=0;
  const store=createStore({query:async(sql,params)=>{
    ...
    if(sql.startsWith('SELECT plan'))return {rows:companies.has(params[0])?[{plan:plans.get(params[0])}]:[]};
    if(sql.startsWith('SELECT modalidad'))return {rows:[{modalidad:modes.get(params[0]),plan:plans.get(params[0])}]};
  ...
  try{
    ...
    plans.set('a','planner');assert.deepEqual((await store.get('a')).productos,['planner']);await assert.rejects(store.set('a','transgest'),{status:409});
    plans.set('a','pro_planner');assert.deepEqual((await store.get('a')).productos,['transgest','planner']);plans.delete('a');
  ...
  for(const module of ['pedidos','clientes','colaboradores','palets','documentos','facturacion','rutas','empresa'])assert.equal(moduleAvailable(['planner'],module),true,module);
  for(const module of ['vehiculos','choferes','gestion_trafico','nominas','taller','ia'])assert.equal(moduleAvailable(['planner'],module),false,module);
  ...
  assert.equal(allowed(['planner'],'facturacion').pass,true);
