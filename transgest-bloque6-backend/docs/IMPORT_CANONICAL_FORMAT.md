...
- `source_sheet` y `source_row` son columnas opcionales de procedencia. Se conservan en la fila original del lote para auditoría y no se aplican como campos operativos. Las demás columnas desconocidas siguen requiriendo un mapeo explícito.
...
- Se admiten libros OOXML con prefijo explícito para el espacio de nombres principal. El lector normaliza ese prefijo en memoria y prescinde de las tablas de formato; no altera el archivo aportado ni los valores de las celdas.
