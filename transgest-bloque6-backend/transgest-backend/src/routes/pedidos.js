...
// ── Portal del proveedor (enlace temporal por viaje) ─────────────────────
// Un unico enlace, sin cuenta ni contrasena, valido hasta 7 dias despues de la
// descarga. A diferencia de los enlaces de accion (de un solo uso), este es
// reutilizable: el proveedor entra las veces que necesite mientras dure el viaje.
// NUNCA muestra lo que se le cobra al cliente: solo lo que se le paga a el.
const PORTAL_PROVEEDOR_DIAS = 7;

async function createColaboradorPortalToken(pedido) {
  await ensureColaboradorWorkflowSchema();
  // Caduca 7 dias despues de la descarga prevista (o de hoy si no hay fecha).
  const ref = normalizePedidoDate(pedido.fecha_descarga || pedido.fecha_entrega || pedido.fecha_carga);
  const base = ref ? new Date(`${ref}T23:59:59Z`) : new Date();
  const horas = Math.max(24, Math.ceil((base.getTime() - Date.now()) / 3600000) + PORTAL_PROVEEDOR_DIAS * 24);
  return createColaboradorToken(pedido, "portal", horas);
}

function portalProveedorUrl(req, token) {
  return `${publicBaseUrl(req)}/api/v1/pedidos/colaborador/portal/${token}`;
}

// Enlace del portal para un pedido (lo usa la app para copiarlo/enviarlo).
router.post("/:id/portal-proveedor", GERENTE_O_TRAFICO, async (req, res) => {
  try {
    const empresaId = req.empresaId || req.user?.empresa_id;
    const { rows } = await db.query(
      "SELECT id, empresa_id, numero, fecha_carga, fecha_descarga, fecha_entrega, colaborador_id FROM pedidos WHERE id=$1 AND empresa_id=$2",
      [req.params.id, empresaId]
    );
    const pedido = rows[0];
    if (!pedido) return res.status(404).json({ error: "Pedido no encontrado" });
    if (!pedido.colaborador_id) return res.status(400).json({ error: "El pedido no tiene proveedor asignado." });
    const token = await createColaboradorPortalToken(pedido);
    await logPedidoEvento(pedido.id, empresaId, "colaborador.portal_enlace_generado", {
      dias_validez: PORTAL_PROVEEDOR_DIAS,
    }, req.user?.rol || "usuario", req.user?.id || null).catch(() => {});
    res.status(201).json({ url: portalProveedorUrl(req, token), dias_validez: PORTAL_PROVEEDOR_DIAS });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

function portalProveedorPage(data, aviso = "") {
  const estado = String(data.estado || "").toLowerCase();
  const rechazado = !!data.colaborador_rechazado_at;
  const paso = (hecho, etiqueta) => `<span style="display:inline-block;margin:0 6px 6px 0;padding:4px 10px;border-radius:999px;font-size:13px;font-weight:700;`
    + `background:${hecho ? "#dcfce7" : "#f1f5f9"};color:${hecho ? "#15803d" : "#64748b"}">`
    + `${hecho ? "Hecho" : "Pendiente"}: ${htmlEscape(etiqueta)}</span>`;
  const aceptado = !!data.colaborador_precio_confirmado;
  return colaboradorPage(`Viaje ${data.numero || ""}`, `
    <h1>Viaje ${htmlEscape(data.numero || "")}</h1>
    <p>${htmlEscape(data.empresa_nombre || "")} te ha asignado este transporte.</p>
    ${aviso ? `<div class="ok">${htmlEscape(aviso)}</div>` : ""}
    ${rechazado ? `<div class="warn">Has rechazado este viaje. Contacta con la empresa si fue un error.</div>` : ""}
    ${renderColaboradorPedidoBox(data, { mostrarPrecio: true })}

    <div class="card">
      <h2>Estado del viaje</h2>
      <p>${paso(aceptado, "Aceptado")} ${paso(!!data.workflow_colaborador_cargado_at, "Cargado")} ${paso(!!data.workflow_colaborador_en_camino_at, "En camino")} ${paso(!!data.workflow_colaborador_descargado_at, "Descargado")}</p>
    </div>

    ${!aceptado && !rechazado ? `
    <div class="card">
      <h2>1. Aceptar el viaje</h2>
      <form method="post" action="aceptar">
        <button type="submit" name="decision" value="aceptar">Acepto el viaje</button>
        <button type="submit" name="decision" value="rechazar">No puedo hacerlo</button>
      </form>
    </div>` : ""}

    <div class="card">
      <h2>2. Tu vehiculo y tu conductor</h2>
      <form method="post" action="asignacion">
        <div class="grid">
          <div><label>Matricula tractora</label><input name="matricula" value="${htmlEscape(data.matricula_colaborador || "")}" required/></div>
          <div><label>Matricula remolque</label><input name="remolque" value="${htmlEscape(data.remolque_matricula_colaborador || "")}"/></div>
          <div><label>Nombre del conductor</label><input name="chofer" value="${htmlEscape(data.conductor_efectivo_nombre || "")}"/></div>
          <div><label>Telefono del conductor</label><input name="telefono" value="${htmlEscape(data.conductor_efectivo_telefono || "")}"/></div>
        </div>
    ...
    <div class="card">
      <h2>3. Marcar avance</h2>
      <form method="post" action="estado">
        <button type="submit" name="paso" value="cargado" ${data.workflow_colaborador_cargado_at ? "disabled" : ""}>He cargado</button>
        <button type="submit" name="paso" value="camino" ${data.workflow_colaborador_en_camino_at ? "disabled" : ""}>En camino</button>
        <button type="submit" name="paso" value="descargado" ${data.workflow_colaborador_descargado_at ? "disabled" : ""}>He descargado</button>
      </form>
    </div>

    <div class="card">
      <h2>4. Documentos (CMR / albaran firmado)</h2>
      <form method="post" action="documento" enctype="multipart/form-data" id="docForm">
        <input type="file" name="archivo" accept="image/*,application/pdf" required/>
        <button type="submit">Subir documento</button>
      </form>
      <p class="muted">Puedes hacer la foto con el movil. Formatos: imagen o PDF (max 5 MB).</p>
    </div>
    <script>
    // El fichero se envia como base64 para no depender de multipart en la API.
    document.getElementById("docForm").addEventListener("submit", function (ev) {
      ...
      r.onload = function () {
        ...
        form.method = "post"; form.action = "documento";
        function add(n, v) { var i = document.createElement("input"); i.type = "hidden"; i.name = n; i.value = v; form.appendChild(i); }
        add("nombre", f.name); add("mime", f.type || "application/octet-stream");
        add("base64", String(r.result).split(",")[1] || "");
...
async function portalProveedorData(token) {
  return getColaboradorTokenData(token, "portal");
}

function portalNoDisponible(res) {
  return res.status(404).send(colaboradorPage("Enlace no disponible",
    `<h1>Enlace no disponible</h1><p>Este enlace ha caducado. Pide uno nuevo a la empresa transportista.</p>`));
}

router.get("/colaborador/portal/:token", async (req, res) => {
  try {
    const data = await portalProveedorData(req.params.token);
    if (!data) return portalNoDisponible(res);
    res.send(portalProveedorPage(data, req.query.ok ? String(req.query.ok).slice(0, 120) : ""));
  } catch (e) { res.status(500).send(colaboradorPage("Error", `<h1>Error</h1><p>${htmlEscape(e.message)}</p>`)); }
});

router.post("/colaborador/portal/:token/aceptar", async (req, res) => {
  try {
    const data = await portalProveedorData(req.params.token);
    if (!data) return portalNoDisponible(res);
    const acepta = String(req.body?.decision || "") === "aceptar";
    if (acepta) {
      await db.query(`
        UPDATE pedidos
         ...
         WHERE id=$1 AND empresa_id=$2`, [data.pedido_id, data.empresa_id]);
    } else {
      await db.query(
        "UPDATE pedidos SET colaborador_rechazado_at=NOW() WHERE id=$1 AND empresa_id=$2",
        [data.pedido_id, data.empresa_id]
      ).catch(() => {});
    }
    await logPedidoEvento(data.pedido_id, data.empresa_id,
      acepta ? "colaborador.portal_aceptado" : "colaborador.portal_rechazado", {}, "colaborador");
    if (!acepta) {
      await notificarGerenciaPedido(data.empresa_id, "colaborador_rechaza",
        "Un proveedor ha rechazado un viaje",
        `${data.colaborador_nombre || "El proveedor"} ha rechazado el viaje ${data.numero || ""}.`,
        { pedido_id: data.pedido_id, dedupe_key: `col_rechaza:${data.pedido_id}` }
      ).catch(() => {});
    }
    res.redirect(303, `../portal/${req.params.token}?ok=${encodeURIComponent(acepta ? "Viaje aceptado. Gracias." : "Has rechazado el viaje.")}`);
  } catch (e) { res.status(500).send(colaboradorPage("Error", `<h1>Error</h1><p>${htmlEscape(e.message)}</p>`)); }
});

router.post("/colaborador/portal/:token/asignacion", async (req, res) => {
  try {
    const data = await portalProveedorData(req.params.token);
    if (!data) return portalNoDisponible(res);
    const matricula = String(req.body?.matricula || "").trim().toUpperCase().slice(0, 60);
    if (!matricula) return res.redirect(303, `../portal/${req.params.token}?ok=${encodeURIComponent("Falta la matricula.")}`);
    await db.query(`
      UPDATE pedidos
         SET matricula_colaborador=$1,
             remolque_matricula_colaborador=$2,
             conductor_efectivo_nombre=$3,
             conductor_efectivo_telefono=$4
       WHERE id=$5 AND empresa_id=$6`,
      [matricula,
       String(req.body?.remolque || "").trim().toUpperCase().slice(0, 60) || null,
       String(req.body?.chofer || "").trim().slice(0, 120) || null,
       String(req.body?.telefono || "").trim().slice(0, 40) || null,
       data.pedido_id, data.empresa_id]);
    await logPedidoEvento(data.pedido_id, data.empresa_id, "colaborador.portal_asignacion", {
      matricula, chofer: String(req.body?.chofer || "").trim().slice(0, 120) || null,
    }, "colaborador");
    res.redirect(303, `../portal/${req.params.token}?ok=${encodeURIComponent("Datos del vehiculo guardados.")}`);
  } catch (e) { res.status(500).send(colaboradorPage("Error", `<h1>Error</h1><p>${htmlEscape(e.message)}</p>`)); }
});

router.post("/colaborador/portal/:token/estado", async (req, res) => {
  try {
    const data = await portalProveedorData(req.params.token);
    if (!data) return portalNoDisponible(res);
    const paso = String(req.body?.paso || "").toLowerCase();
    const mapa = {
      cargado: { col: "workflow_colaborador_cargado_at", estado: "en_curso", texto: "Carga registrada." },
      camino: { col: "workflow_colaborador_en_camino_at", estado: "en_curso", texto: "Viaje en camino." },
      descargado: { col: "workflow_colaborador_descargado_at", estado: "descarga", texto: "Descarga registrada." },
    };
    const cfg = mapa[paso];
    if (!cfg) return portalNoDisponible(res);
    await db.query(`
      UPDATE pedidos
         SET ${cfg.col}=NOW(),
             estado=CASE WHEN LOWER(estado::text) IN ('pendiente','confirmado','en_curso')
                         THEN $1::estado_pedido ELSE estado END
       WHERE id=$2 AND empresa_id=$3`, [cfg.estado, data.pedido_id, data.empresa_id]);
    await logPedidoEvento(data.pedido_id, data.empresa_id, `colaborador.portal_${paso}`, {}, "colaborador");
    res.redirect(303, `../portal/${req.params.token}?ok=${encodeURIComponent(cfg.texto)}`);
  } catch (e) { res.status(500).send(colaboradorPage("Error", `<h1>Error</h1><p>${htmlEscape(e.message)}</p>`)); }
});

router.post("/colaborador/portal/:token/documento", async (req, res) => {
  try {
    const data = await portalProveedorData(req.params.token);
    if (!data) return portalNoDisponible(res);
    const base64 = String(req.body?.base64 || "");
    if (!base64 || base64.length > 7000000) {
      return res.redirect(303, `../portal/${req.params.token}?ok=${encodeURIComponent("No se pudo subir el archivo (vacio o demasiado grande).")}`);
    }
    await db.query(
      `INSERT INTO pedido_docs (pedido_id,empresa_id,nombre,tipo,file_base64,file_mime,file_size_kb,notas)
       VALUES ($1,$2,$3,'albaran',$4,$5,$6,'Subido por el proveedor desde su enlace')`,
      [data.pedido_id, data.empresa_id,
       String(req.body?.nombre || "Documento proveedor").slice(0, 180),
       base64, String(req.body?.mime || "application/octet-stream").slice(0, 100),
       Math.round(base64.length * 0.75 / 1024)]
    );
    await logPedidoEvento(data.pedido_id, data.empresa_id, "colaborador.portal_documento", {
      nombre: String(req.body?.nombre || "").slice(0, 180),
    }, "colaborador");
    res.redirect(303, `../portal/${req.params.token}?ok=${encodeURIComponent("Documento subido. Gracias.")}`);
  } catch (e) { res.status(500).send(colaboradorPage("Error", `<h1>Error</h1><p>${htmlEscape(e.message)}</p>`)); }
});
