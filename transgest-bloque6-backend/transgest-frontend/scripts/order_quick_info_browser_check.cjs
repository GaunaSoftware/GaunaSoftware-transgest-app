...
await load.hover();await page.getByRole('tooltip').filter({hasText:'Carga adicional Toledo'}).waitFor();await page.keyboard.press('Escape');
const unload=row.getByRole('button').filter({hasText:'+1 descarga'});
await unload.hover();await page.getByRole('tooltip').filter({hasText:'Descarga adicional Alicante'}).waitFor();await page.keyboard.press('Escape');
