...
## Corrección tras el primer despliegue BI (23/09/2026)

La lectura real de `/bi/workspace` y `/bi/analitica` devolvió 500, mientras `/bi/resumen` y `/health` respondían 200. Los logs de Render identificaron `42P01: relation "gastos_estructura" does not exist`. Esa tabla procede de una instalación opcional del esquema antiguo y no existe en esta base productiva. `loadAnalyticsSources` ahora la consulta como fuente opcional, igual que repostajes y nóminas: el servicio continúa, añade `gastos_estructura` a `fuentes_no_disponibles` y mantiene el importe de estructura como `null`, no como cero confirmado. La regresión simula la tabla ausente tanto en lectura normal como en snapshot de solo lectura. No se creó la tabla ni se tocaron datos financieros históricos.
...
## Ajuste móvil BI y diagnóstico de entrega (24/09/2026)

La captura de iPhone mostró las pestañas BI superpuestas: cada botón flex se encogía aunque su texto no pudiera partirse. La fila mantiene ahora el ancho de cada botón, permite desplazamiento horizontal y centra la pestaña activa cuando la vista se restaura o cambia. El gráfico de ingresos y margen muestra una leyenda textual junto a sus colores, tanto en el panel como en la ampliación; la cascada también identifica ingresos, costes y acumulado.

El destinatario indicó que la prueba del 23/09 no apareció en la bandeja. La respuesta anterior acreditaba únicamente la aceptación por el servidor SMTP. La prueba de SuperAdmin usaba una invitación con enlace de contraseña; se sustituyó por un mensaje diagnóstico sencillo con versiones de texto y HTML, sin enlace ni imagen, y la interfaz deja de afirmar entrega: muestra destinatario aceptado e identificador del mensaje para rastreo. `npm run security:regression`, `npm run bi:regression`, las 79 pruebas React y el build frontend pasaron localmente; el build conserva advertencias previas ajenas a estos cambios. La prueba local usa un servidor SMTP aislado, por lo que la recepción real requiere una nueva prueba después del despliegue y, si no aparece, el seguimiento del mensaje en el correo destinatario.

Una vista HTML aislada con el CSS real y datos **sintéticos rotulados** se midió en navegador a 390, 402 (iPhone 16 Pro) y 768 px. Ninguna pestaña se solapó con la siguiente; el documento midió 375/387/753 px respectivamente, por debajo del viewport. La navegación quedó desplazable internamente (882 px de contenido), sin provocar desplazamiento horizontal de toda la página. La lista accesible de series contiene las dos etiquetas del gráfico. Esta comprobación de CSS no sustituye una captura en Safari físico; se repetirá la inspección en la aplicación desplegada.
