...
- La previsualizacion y aplicacion controlada de cuentas contables reutiliza el mismo endpoint con `import_type=accounts` y `fiscal_year_id` explicito. Crea cuentas solo en el ejercicio seleccionado, bloquea codigos duplicados y rechaza la aplicacion si el ejercicio destino no esta abierto.
