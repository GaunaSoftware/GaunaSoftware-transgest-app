...
import QuickInfo from "./OrderQuickInfo";
import { driverName, orderRig, incidentDescription, incidentLabel } from "./quickInfo";
...
export default function OrdersWorkspace({ vehicles = [], items, allItems, loading, error, reload, clients, drivers, labels, filters, actions, permissions, selectedIds, toggleSelected, tools, bulkTools, serverPage, serverPages, totalCount, setServerPage, describe }) {
  const [actionPanel,setActionPanel]=useState(null);
  const [expandedId,setExpandedId]=useState(null);
  const [asideCollapsed,setAsideCollapsed]=useState(()=>{try{return localStorage.getItem('tg_orders_aside_collapsed')==='1';}catch{return false;}});
  function toggleAside(){setAsideCollapsed(value=>{try{localStorage.setItem('tg_orders_aside_collapsed',value?'0':'1');}catch{}return !value;});}
  const toggleDetails=p=>setExpandedId(id=>id===String(p.id)?null:String(p.id));
  const [detailResult,setDetailResult]=useState({});
  const [detailRetry,setDetailRetry]=useState(0);
  const expandedVersion=items.find(item=>String(item.pedido.id)===expandedId)?.pedido.updated_at;
  useEffect(()=>{
    if(!expandedId)return;
    let alive=true;
    setDetailResult({id:expandedId});
    getPedido(expandedId).then(data=>{if(alive)setDetailResult({id:expandedId,data});}).catch(()=>{if(alive)setDetailResult({id:expandedId,error:true});});
    return ()=>{alive=false;};
  },[expandedId,expandedVersion,detailRetry]);
  const details=p=>expandedId===String(p.id)?<OrderInlineDetails key={p.id} pedido={p} data={detailResult.id===expandedId?detailResult.data:null} error={detailResult.id===expandedId&&detailResult.error} onRetry={()=>setDetailRetry(v=>v+1)} vehicles={vehicles} drivers={drivers} labels={labels} describe={describe}/>:null;
  ...
  function panel(p,kind){
    const groups={
      ...
      'Documentos':[{label:'Documento de control digital (DCD)',onClick:()=>actions.dcd(p)},{label:'Carta de porte / CMR',onClick:()=>actions.letter(p)},...(canOrder(p)?[{label:'Orden de carga',onClick:()=>actions.order(p)}]:[]),...(permissions.invoice&&p.estado==='entregado'&&!isLocked(p)&&!permissions.draftInvoice(p)?[{label:'Facturar',onClick:()=>actions.invoice(p)}]:[])],
  ...
  const rowActions=p=><div className="orders-row-actions"><Button className="orders-summary-toggle" aria-label={`Resumen de ${p.numero}`} aria-expanded={expandedId===String(p.id)} onClick={()=>toggleDetails(p)}>{expandedId===String(p.id)?"Ocultar":"Resumen"}</Button><Button aria-label={`Ver pedido ${p.numero}`} onClick={()=>actions.open(p)}>Ver</Button><DropdownMenu label={`Acciones de ${p.numero}`} items={[
  ...
  const columns=[{key:"selection",label:<span className="orders-sr">Selección</span>,render:({pedido:p})=>check(p)},
    {key:"number",label:"Pedido",className:"orders-number-cell",render:({pedido:p})=><button className="orders-link" onClick={()=>actions.open(p)}>{p.numero || "Sin número"}</button>},
    {key:"client",label:"Cliente",className:"orders-client-cell",render:({pedido:p})=><>{p.cliente_nombre || clients.find(c=>String(c.id)===String(p.cliente_id))?.nombre || "—"}</>},
    {key:"origin",label:"Origen",className:"orders-place-cell",render:({pedido:p})=>{const r=describe(p);return <QuickInfo label={<>{r.origin}{r.loads>1&&<span className="orders-stop-count">+{r.loads-1} {r.loads===2?"carga":"cargas"}</span>}</>} text={r.loadDetails || r.origin}/>;}},
    {key:"destination",label:"Destino",className:"orders-place-cell",render:({pedido:p})=>{const r=describe(p);return <QuickInfo label={<>{r.destination}{r.unloads>1&&<span className="orders-stop-count">+{r.unloads-1} {r.unloads===2?"descarga":"descargas"}</span>}</>} text={r.unloadDetails || r.destination}/>;}},
    {key:"date",label:"Fecha carga",className:"orders-date-cell",render:({pedido:p})=><span className="orders-date">{dateLabel(p.fecha_carga)}<small>{String(p.hora_carga||"").slice(0,5)}</small></span>},
    {key:"unloadDate",label:"Fecha descarga",className:"orders-date-cell",render:({pedido:p})=><span className="orders-date">{dateLabel(p.fecha_descarga || p.fecha_entrega)}<small>{String(p.hora_descarga||"").slice(0,5)}</small></span>},
    {key:"vehicle",label:"Matrícula",className:"orders-vehicle-cell",render:({pedido:p})=><>{orderRig(p,vehicles)}{p.colaborador_id&&<small>{p.colaborador_nombre || "Colaborador"}</small>}</>},
    {key:"driver",label:"Conductor",className:"orders-driver-cell",render:({pedido:p})=>driver(p)},
    {key:"state",label:"Estado",className:"orders-state-cell",render:({pedido:p})=><><span title={incidentDescription(p)}><Badge tone={stateTone(p.estado)}>{labels[p.estado] || p.estado || "Sin estado"}</Badge></span>{isLocked(p)&&<small>Facturado</small>}{p.estado==="cancelado"&&p.motivo_cancelacion&&<small>{p.motivo_cancelacion}</small>}</>},
    {key:"incident",label:"Incidencia",className:"orders-incident-cell",render:({pedido:p,priorityMeta:m})=>incidentLabel(p)?<QuickInfo label={<span className="orders-incident"><Icon name="alert" size={14}/>{incidentLabel(p)}</span>} text={incidentDescription(p)}/>:m.validationIssues.length?<QuickInfo label={<span className="orders-warning">Datos pendientes ({m.validationIssues.length})</span>} text={m.validationIssues.join("\n")}/>:"—"},
  ...
  return <Page className="orders-workspace">{actionPanel&&<Modal title={actionPanel.title} onClose={()=>setActionPanel(null)}><div className="order-action-choices">{actionPanel.items.map(item=><Button key={item.label} onClick={()=>{setActionPanel(null);item.onClick();}}>{item.label}</Button>)}</div></Modal>}<PageHeader title="Pedidos / Tráfico" description="Control y seguimiento de tus pedidos de transporte." actions={<><span className="orders-today">{new Date().toLocaleDateString("es-ES",{day:"numeric",month:"long",year:"numeric"})}</span>{permissions.edit&&<Button variant="primary" onClick={actions.new}>+ Nuevo pedido</Button>}<DropdownMenu label="Opciones de pedidos" items={[...(permissions.edit?[{label:"Pedido rápido",onClick:actions.quick}]:[]),{label:"Planificación y bandeja IA",onClick:()=>setShowTools(v=>!v)}]}/></>}/>
    ...
    <div className={`orders-layout ${asideCollapsed?"orders-layout--collapsed":""}`}><div className="orders-main"><div className="orders-kpis">{kpis.map(k=><KpiCard key={k.label} {...k} value={loading||error?"—":k.value} detail="En el listado cargado"/>)}</div>
      <Card className="orders-list-card"><FilterBar search={<SearchInput label="Buscar pedidos" value={filters.q} onChange={e=>filters.setQ(e.target.value)} placeholder="Buscar por pedido, cliente, origen, destino…"/>} advanced={<>
        ...
        {error?<div className="orders-error" role="alert"><p>No se han podido cargar los pedidos.</p><Button onClick={reload}>Reintentar</Button></div>:<DataTable loading={loading} rows={rows} rowKey={i=>i.__group?i.id:i.pedido.id} renderGroup={g=><div className={`orders-date-group orders-date-group-${g.level}`}><strong>{g.label}</strong>{g.count!=null&&<span>{g.count} pedidos en el listado filtrado</span>}</div>} rowId={i=>`order-compact-${i.pedido.id}`} columns={columns} renderExpanded={({pedido:p})=>details(p)} emptyTitle="No hay pedidos con estos filtros" rowClassName={i=>!i.__group&&selectedIds.includes(String(i.pedido.id))?"orders-selected":""} renderMobile={({pedido:p,priorityMeta:m})=><MobileDataCard title={<button className="orders-link" onClick={()=>actions.open(p)}>{p.numero}</button>} subtitle={p.cliente_nombre} actions={<>{check(p)}{rowActions(p)}</>}><QuickInfo label={<>{describe(p).origin} → {describe(p).destination}</>} text={`Cargas:\n${describe(p).loadDetails || describe(p).origin}\nDescargas:\n${describe(p).unloadDetails || describe(p).destination}`}/><span>Carga: {dateLabel(p.fecha_carga)} {String(p.hora_carga||"").slice(0,5)}</span><span>Descarga: {dateLabel(p.fecha_descarga || p.fecha_entrega)} {String(p.hora_descarga||"").slice(0,5)}</span><span>{orderRig(p,vehicles)} · {driver(p)}</span>{p.colaborador_id&&<span>Colaborador: {p.colaborador_nombre || "Asignado"}</span>}<Badge tone={stateTone(p.estado)}>{labels[p.estado]||p.estado}</Badge>{incidentLabel(p)&&<QuickInfo label={<span className="orders-incident"><Icon name="alert" size={14}/>{incidentLabel(p)}</span>} text={incidentDescription(p)}/>}{m.validationIssues.length>0&&<small>{m.validationIssues.length} datos pendientes</small>}{details(p)}</MobileDataCard>}/>}
    ...
    </div><div className="orders-panel-divider"><Button className="orders-panel-toggle" aria-label={asideCollapsed?"Mostrar panel de seguimiento":"Plegar panel de seguimiento"} title={asideCollapsed?"Mostrar panel de seguimiento":"Plegar panel de seguimiento"} aria-expanded={!asideCollapsed} aria-controls="orders-follow-up" onClick={toggleAside}><span aria-hidden="true">{asideCollapsed?"‹":"›"}</span><span className="orders-panel-toggle-label">{asideCollapsed?"Mostrar seguimiento":"Plegar seguimiento"}</span></Button></div><aside id="orders-follow-up" hidden={asideCollapsed} className="orders-aside" aria-label="Seguimiento de tráfico">
