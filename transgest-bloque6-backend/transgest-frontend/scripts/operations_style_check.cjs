...
const root=path.resolve(__dirname,'../build'),out=path.join(root,'qa','operations');
...
async function main(){fs.mkdirSync(out,{recursive:true});
 ...
 try {
  ...
  for(const [view,title,cls] of [['gestion_trafico','Mesa de tráfico','traffic'],['solicitudes','Peticiones de viaje','requests'],['control_tower','Control Tower','tower'],['calculador_portes','Calculador de portes','calculator']]){
    await page.evaluate(v=>window.dispatchEvent(new CustomEvent('tms:navegar',{detail:v})),view);
    const panel=page.locator('.operations-'+cls);await panel.getByRole('heading',{name:title,exact:true}).waitFor();
    ...
    for(const theme of ['light','dark']){
      ...
      for(const width of [390,768,1672]){
        ...
        assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth+1),false,`${view} overflow ${width}`);
        await page.screenshot({path:path.join(out,`${cls}-${theme}-${width}.png`),fullPage:true});
      }
    }
    checks.push(view+' light/dark mobile/tablet/desktop');
  }
