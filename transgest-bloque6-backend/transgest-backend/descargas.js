...
router.get("/pedidos/:id/descargas", async (req, res) => {
  const { rows } = await db.query(
    "SELECT * FROM pedido_descargas WHERE pedido_id=$1 ORDER BY orden,created_at",
    [req.params.id]
  ).catch(()=>({rows:[]}));
  res.json(rows);
});
...
router.patch("/pedidos/:id/descargas/:did", GERENTE_O_TRAFICO, async (req, res) => {
  ...
  const { rows } = await db.query(`UPDATE pedido_descargas SET ${fields.join(',')} WHERE id=$${i} RETURNING *`, vals).catch(e=>({rows:[]}));
  res.json(rows[0]||{});
});

// DELETE /pedidos/:id/descargas/:did
router.delete("/pedidos/:id/descargas/:did", GERENTE_O_TRAFICO, async (req, res) => {
  await db.query("DELETE FROM pedido_descargas WHERE id=$1", [req.params.did]).catch(()=>{});
  res.json({ ok: true });
});
...
router.patch("/grupajes/:id", GERENTE_O_TRAFICO, async (req, res) => {
  ...
  const { rows } = await db.query(
    `UPDATE grupajes SET nombre=COALESCE($1,nombre), vehiculo_id=COALESCE($2,vehiculo_id), chofer_id=COALESCE($3,chofer_id),
     ...
     kg_disponible=COALESCE($7,kg_disponible), notas=COALESCE($8,notas) WHERE id=$9 RETURNING *`,
    [nombre, vehiculo_id, chofer_id, fecha, estado, metros_libres, kg_disponible, notas, req.params.id]
  ).catch(()=>({rows:[]}));
  res.json(rows[0]||{});
});
...
router.post("/grupajes/:id/pedidos", GERENTE_O_TRAFICO, async (req, res) => {
  ...
  await db.query("UPDATE pedidos SET grupaje_id=$1 WHERE id=$2", [req.params.id, pedido_id]).catch(()=>{});
  res.json({ ok: true });
});

// Quitar pedido de grupaje
router.delete("/grupajes/:id/pedidos/:pid", GERENTE_O_TRAFICO, async (req, res) => {
  await db.query("UPDATE pedidos SET grupaje_id=NULL WHERE id=$1", [req.params.pid]).catch(()=>{});
  res.json({ ok: true });
});
