...
export default function PlannerLoads({onPlan}) {
  ...
  return <section className="planner-loads">
    <div className="planner-toolbar"><h1>Cargas</h1><p>Asigna un proveedor o planifica el transporte con tu flota en Gestión de viajes.</p><label>Mes<input type="month" value={month} required onChange={e=>{if(e.target.value){setMonth(e.target.value);setPage(1);}}}/></label><button disabled={loading} onClick={load}>Actualizar</button>{canEdit && <button className="primary" onClick={()=>open(empty())}>Nueva carga</button>}</div>
