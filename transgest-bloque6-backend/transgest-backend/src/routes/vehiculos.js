...
r1.put('/:id/imagen', GERENTE_O_TRAFICO, async (req,res) => {
  try {
    const row=await require('../services/vehicleImage').saveVehicleImage(db,{id:req.params.id,empresaId:req.empresaId || req.user.empresa_id,image:req.body?.imagen_data});
    res.json(row);
