...
async function main(){
 ...
 try {
  ...
  await page.route('**/api/v1/**',async route=>{
   ...
   else if(pathname==='/mi-cuenta/')data={nombre:'TransGest QA',plan:'enterprise',estado:'activa'};
  ...
  assert.equal(await page.getByRole('tab',{name:'Resumen',exact:true}).getAttribute('aria-selected'),'true');
  await page.locator('.finance-summary-invoices .tgui-table tbody tr').first().waitFor();
  assert.equal(await page.locator('.finance-summary-invoices .tgui-table tbody tr').count(),5);
  assert.equal(await page.getByLabel('Mes de facturación').count(),1);
  assert.equal(await page.locator('.finance-summary-followup > section').count(),3);
  const chartBox=await page.locator('.finance-summary-chart').boundingBox(), backlogBox=await page.locator('.finance-summary-backlog').boundingBox(), invoiceBox=await page.locator('.finance-summary-invoices').boundingBox(), followupBox=await page.locator('.finance-summary-followup').boundingBox();
  assert.ok(Math.abs(chartBox.y-backlogBox.y)<2);
  assert.ok(invoiceBox.y>=chartBox.y+chartBox.height);
  assert.ok(followupBox.y>=invoiceBox.y+invoiceBox.height);
  await page.getByRole('button',{name:'Página siguiente del resumen',exact:true}).click();
  assert.equal(await page.locator('.finance-summary-invoices .tgui-table tbody tr').count(),3);
  await page.getByLabel('Facturas por página en resumen').selectOption('10');
  assert.equal(await page.locator('.finance-summary-invoices .tgui-table tbody tr').count(),8);
  await page.getByLabel('Facturas por página en resumen').selectOption('5');
  await page.getByLabel('Cliente del resumen').selectOption(client.id);
  await page.locator('.finance-summary-invoices').getByRole('searchbox').fill('sin coincidencias');
  await page.getByText('Sin facturas',{exact:true}).waitFor();
  assert.equal(await page.getByLabel('Cliente del resumen').inputValue(),client.id,'Client filter remains visible when search is empty');
  await page.locator('.finance-summary-invoices').getByRole('searchbox').fill('');
  await page.getByLabel('Cliente del resumen').selectOption('');
  await page.locator('.finance-summary-invoices').getByRole('searchbox').fill('A-2026-0066');
  await page.waitForFunction(()=>document.querySelectorAll('.finance-summary-invoices .tgui-table tbody tr').length===1);
  await page.locator('.finance-summary-invoices').getByRole('searchbox').fill('');
  await page.getByRole('button',{name:'Exportar',exact:true}).click();
  await page.getByRole('dialog',{name:'Exportación contable',exact:true}).waitFor();
  await page.keyboard.press('Escape');
  await page.getByRole('button',{name:'Revisar documentos',exact:true}).click();
  await page.getByRole('dialog',{name:'Bloqueos documentales',exact:true}).waitFor();
  await page.keyboard.press('Escape');
  await page.getByRole('tab',{name:'Resumen',exact:true}).click();
  await page.getByRole('button',{name:'Revisar viajes',exact:true}).click();
  await page.getByRole('dialog',{name:'Viajes entregados sin facturar',exact:true}).waitFor();
  await page.keyboard.press('Escape');
  checks.push('summary-default','summary-composition','summary-pagination','summary-client-filter','summary-search','summary-export','summary-document-link','summary-backlog-link');
  await page.getByRole('tab',{name:'Resumen',exact:true}).click();
  await page.setViewportSize({width:1536,height:1024});
  await page.locator('.tg-content').evaluate(el=>{el.scrollTop=0;});
  await page.screenshot({path:path.join(out,'reference-1536-Resumen.png')});
  ...
  for(const theme of ['light','dark']){
   ...
   for(const width of [390,430,767,768,1024,1366,1440,1600,1920]){
    ...
    for(const tab of ['Resumen','Facturas','Cobros','Pagos','Tesorería','Fiscal (AEAT)']){
     ...
     if(tab==='Facturas') {
      assert.equal(await page.locator('.tgui-mobile-data:visible').count(),width<768?1:0);
      if(width>=1366)assert.ok((await page.locator('.tgui-table tbody tr').first().boundingBox()).height<=72);
     }
     assert.equal(await page.locator('.finance-kpis .tgui-kpi').count(),4,'One contextual KPI row');
     if(tab==='Tesorería')assert.equal(await page.getByText('Saldo previsto 30 días',{exact:true}).count(),0,'No duplicate treasury KPI');
  ...
  assert.equal(await page.locator('.tg-sidebar .tg-sidebar-user-copy').count(),0);
  assert.equal(await page.locator('.tg-user-chip').count(),1);
  for(const name of ['Contactar soporte','Cerrar sesión']) {
  ...
  assert.equal(await page.evaluate(()=>localStorage.getItem('tms_token')),null);
  checks.push('logout-mobile');
