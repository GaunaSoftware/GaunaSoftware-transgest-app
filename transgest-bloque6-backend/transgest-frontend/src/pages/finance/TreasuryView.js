...
export default function TreasuryView({ forecast, money, date, onReport }) {
  ...
  return <Section title="Tesorería" actions={<Button onClick={onReport}>Informe tesorería</Button>}>
    <div className="finance-treasury-kpis"><KpiCard label="Saldo actual" value={`${money(forecast.capitalActual)} €`} detail="Se modifica desde Mi Empresa > Tesorería" /><KpiCard label="Saldo previsto 30 días" value={`${money(forecast.saldoPrevisto30)} €`} tone={forecast.saldoPrevisto30 < 0 ? "danger" : "neutral"} detail={`Capital + neto de ${money(forecast.neto30)} €`} /></div>
